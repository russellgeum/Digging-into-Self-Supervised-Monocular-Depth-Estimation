...
class KITTIMonoDataset(Dataset):
    def __init__(self, datapath, filename, is_training, frame_ids, ext, height, width, scale = 4):
        ...
        Image.ANTIALIAS는 https://pillow.readthedocs.io/en/stable/reference/Image.html#PIL.Image.ANTIALIAS를 참고
        """
        ...
        self.scale_pyramid = [(self.height//(2**i), self.width//(2**i)) for i in self.scales]

        for scale, (height, width) in enumerate(self.scale_pyramid):
        ...
        print(">>> KITTI Dataloader information")
    ...
    def preprocessing_point(self, input_data, folder_name, key_frame, side, do_flip):
        ...
        input_data.update({("depth", 0): self.load_point(calib_path, point_path, side, do_flip)})
        return input_data
    ...
    def preprocessing_intrinsic(self, input_data):
        ...
        for scale in self.scales:
            K_copy       = self.K.copy()
            K_copy[0, :] = K_copy[0, :] * self.scale_pyramid[scale][1]
            K_copy[1, :] = K_copy[1, :] * self.scale_pyramid[scale][0]
            inv_K        = np.linalg.pinv(K_copy)

            input_data.update({("K", scale): torch.from_numpy(K_copy)})
            input_data.update({("inv_K", scale): torch.from_numpy(inv_K)})
        return input_data
