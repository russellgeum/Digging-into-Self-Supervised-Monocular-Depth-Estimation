...
class trainer(object):
    def __init__(self, options):
        ...
        self.use_automasking  = options.use_automasking
    ...
    def definition_model(self, model_dict, param_list):
        # 뎁스 인코더, 디코더, 포즈 네트워크, 마스크 네트워크 로드하는 함수
        model_dict["encoder"] = ResnetEncoder(num_layers = self.num_layers, pretrained = self.weight_init)
        model_dict["decoder"] = DepthDecoder(num_ch_enc = self.model["encoder"].num_ch_enc, scales = self.scales)
        
        if self.pose_type == "posecnn":
            model_dict["pose_decoder"] = PoseCNN(num_input_frames = self.num_pose_frames)

        if self.pose_type == "shared":
            model_dict["pose_decoder"] = PoseDecoder(model_dict["encoder"].num_ch_enc, self.num_pose_frames)

        elif self.pose_type == "separate":
            model_dict["pose_encoder"] = ResnetEncoder(self.num_layers, self.weight_init, self.num_pose_frames)
            model_dict["pose_decoder"] = PoseDecoder(
                                            model_dict["pose_encoder"].num_ch_enc, 
                                            num_input_features = 1, 
                                            num_frames_to_predict_for = 2)

        model_dict["encoder"].to(self.device)
        model_dict["decoder"].to(self.device)
        if self.pose_type == "separate":
            model_dict["pose_encoder"].to(self.device)
        model_dict["pose_decoder"].to(self.device)

        param_list = param_list + list(model_dict["encoder"].parameters())
        param_list = param_list + list(model_dict["decoder"].parameters())
        if self.pose_type == "separate":
            param_list = param_list + list(model_dict["pose_encoder"].parameters())
    ...
    def compute_loss(self, inputs, outputs): # 로스 계산
        ...
        4. (매우 중요) self.use_automasking을 켜서 no_reproejction_loss 계산
            reproejction_loss warping한 이미지와 target 사이의 로스를 계산
            no_reproejction_loss warping하지 않은 원본과 타겟을 계산 

            reproejction_loss < no_reproejction_loss
        ...
        for scale in self.scales: # 0 1 2 3
            ...
            for frame_id in self.frame_ids[1: ]:
                ...
                reproejction_loss.append(warp_loss)
            reproejction_loss = torch.cat(reproejction_loss, 1)
            
            # if self.use_automasking(True) == if not self.disable_automasking(False)
            if self.use_automasking:
                no_reproejction_loss = []
                for frame_id in self.frame_ids[1: ]:
